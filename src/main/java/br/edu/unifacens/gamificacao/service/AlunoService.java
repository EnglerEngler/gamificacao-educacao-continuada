...
public class AlunoService {
    ...
    public AlunoResponse criar(CriarAlunoRequest request) {
        return AlunoResponse.of(
                repository.save(new AlunoEntity(request.nome(), request.cursosDisponiveis()))
        );
    }

    public List<AlunoResponse> listar() {
        return repository.findAll().stream().map(AlunoResponse::of).toList();
    }
    ...
    public AlunoResponse concluirCurso(Long id, ConcluirCursoRequest request) {
        ...
        entity.setCursosConcluidos(aluno.getCursosConcluidos());
        entity.setPlano(aluno.getPlano());
        entity.setMoedas(aluno.getMoedas());
